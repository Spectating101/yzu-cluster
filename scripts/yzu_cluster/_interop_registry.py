...
class RegistryMixin:
    def register(self, run_id: str, *, dataset_id: str, registry_id: str, manifest_id: str,
        ...
        with self.transaction():
            self.db.execute("""INSERT INTO assets VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
              ...
              (dataset_id,registry_id,revision_id,title,ready,verification_state,verification_summary,dumps(dict(source_obj)),
               dumps(lineage),dumps(ids(source_snapshots)),manifest_id,checksum,method_revision,vault_path,1,refresh_policy,
               last_refreshed_at,next_refresh_at,0,rows,fields,entities,grain,coverage,at))
            self.db.execute("""UPDATE runs SET stage='registered',outputs=?,manifest_id=?,archive_verified=1,registry_id=?,
            ...
            self._event(run_id,"registered","registered",at=at,worker_id=row["worker_id"],attempt=row["attempt"],
                        payload={"dataset_id":dataset_id,"registry_id":registry_id,"manifest_id":manifest_id,
    ...
    def asset(self, dataset_id: str) -> dict[str, Any]:
        ...
        return {"dataset_id":row["dataset_id"],"registry_id":row["registry_id"],"revision_id":row["revision_id"],
                ...
                "lineage":{"inputs":loads(row["lineage_inputs"],[]),"source_snapshots":loads(row["source_snapshots"],[])},
                "manifest_id":row["manifest_id"],"checksum":row["checksum"],"method_revision":row["method_revision"],
                "vault_path":row["vault_path"],"drive_verified":bool(row["archive_verified"]),
                "refresh_policy":row["refresh_policy"],"last_refreshed_at":row["last_refreshed_at"],
                "next_refresh_at":row["next_refresh_at"],"stale":bool(row["stale"]),
                "row_count":row["rows_count"],"field_count":row["fields_count"],"entity_count":row["entities_count"],
    ...
    def snapshot(self, run_id: str) -> dict[str, Any]:
        ...
        if row["progress_current"] is not None or row["progress_total"] is not None:
            progress = {key:value for key,value in (("current",row["progress_current"]),("total",row["progress_total"])) if value is not None}
        worker = self.worker(row["worker_id"]) if row["worker_id"] else None
        events = [{"event_type":event["event_type"],"stage":event["stage"],"timestamp":event["timestamp"],
                   "worker_id":event["worker_id"],"attempt":event["attempt"],"message":event["message"],
                   **loads(event["payload"],{})}
                  for event in self.db.execute("SELECT * FROM events WHERE run_id=? ORDER BY event_id",(run_id,)).fetchall()]
        inputs_, outputs_ = loads(row["inputs"],[]), loads(row["outputs"],[])
        result = {"id":row["job_id"],"run_id":row["run_id"],"type":row["job_type"],"name":row["title"],
                  "status":row["stage"],"attempt":row["attempt"],"max_attempts":row["max_attempts"],
                  "retryable":bool(row["retryable"]),"required_capabilities":loads(row["required_capabilities"],[]),
                  "assigned_worker":worker,"worker_pool":row["pool"],"lease_expires_at":row["lease_expires_at"],
        ...
        result["lifecycle"] = {"stage":row["stage"],"progress":progress,"worker":worker,"attempt":row["attempt"],
                               "inputs":inputs_,"outputs":outputs_,"events":events,"run_id":row["run_id"],
                               "started_at":row["started_at"],"finished_at":row["finished_at"]}
        result["execution"] = {"run_id":row["run_id"],"stage":row["stage"],"worker":worker,"pool":row["pool"],
    ...
    def active(self, limit: int = 100) -> list[dict[str, Any]]:
        rows = self.db.execute("""SELECT run_id FROM runs WHERE stage NOT IN('completed','registered') ORDER BY
          ...
          WHEN 'retrying' THEN 2 WHEN 'queued' THEN 3 ELSE 9 END,updated_at DESC LIMIT ?""",(max(0,limit),)).fetchall()
