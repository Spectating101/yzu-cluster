...
export function normalizeAssetAuthority(asset = {}) {
  ...
  return {
    ...
    refresh: {
      ...
      last_refreshed_at: firstValue(refresh.last_refreshed_at, asset?.last_refreshed_at, asset?.data_refreshed_at, asset?.refreshed_at) || null,
      data_as_of: firstValue(refresh.data_as_of, refresh.as_of, asset?.data_as_of, asset?.as_of) || null,
      next_refresh_at: firstValue(refresh.next_refresh_at, asset?.next_refresh_at) || null,
      status: firstValue(refresh.status, asset?.refresh_status, asset?.pipeline_status) || null,
      stale: truthy(firstValue(refresh.stale, asset?.stale)),
      record_updated_at: firstValue(asset?.updated_at, asset?.last_modified) || null,
    },
    shape: {
...
export function assetAuthorityContext(asset = {}) {
  ...
  return {
    ...
    data_as_of: authority.refresh.data_as_of || undefined,
    next_refresh_at: authority.refresh.next_refresh_at || undefined,
    refresh_status: authority.refresh.status || undefined,
    stale: authority.refresh.stale || undefined,
    record_updated_at: authority.refresh.record_updated_at || undefined,
