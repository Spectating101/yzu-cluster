...
test("seed endpoint failure does not block the existing cold-start Home", async ({ page }) => {
  await mockV2Api(page, {
    datasetsBody: { datasets: [] },
    jobsBody: { jobs: [] },
    healthBody: emptyHealth(),
    profileBody: { found: false, profile: { email: "researcher@example.test", unknown: true } },
  });
  await page.route("**/library/seed", (route) =>
    route.fulfill({ status: 404, contentType: "application/json", body: JSON.stringify({ error: "not available" }) }),
  );
  await page.goto("/?tab=home", { waitUntil: "domcontentloaded" });
  await waitForShell(page);
  ...
  await expect(seed).toContainText("Suggested asks");
