...
function put(grid, x, y, text) {
  if (y < 0 || y >= grid.length) return;
  for (let i = 0; i < text.length; i += 1) {
    const cx = x + i;
    if (cx >= 0 && cx < grid[y].length) grid[y][cx] = text[i];
  }
}
function box(grid, x, y, w, h, label) {
  if (w < 2 || h < 2) return;
  const right = x + w - 1, bottom = y + h - 1;
  put(grid, x, y, "+" + "-".repeat(Math.max(0, w - 2)) + "+");
  put(grid, x, bottom, "+" + "-".repeat(Math.max(0, w - 2)) + "+");
  for (let row = y + 1; row < bottom; row += 1) { put(grid, x, row, "|"); put(grid, right, row, "|"); }
  if (label) put(grid, x + 2, y, " " + label.slice(0, Math.max(0, w - 6)) + " ");
}

test("wireframe from the real layout", async ({ page }) => {
  const thread = {
  ...
  const nodes = await page.evaluate(() => {
    const wanted = [
      ["panel", "[data-testid^='synthesis-']"],
      ["head", ".s04-title h2"],
      ["eyebrow", ".s04-title small"],
      ["badge", ".s04-title em"],
      ["group", ".s04-options > small"],
      ["btn", ".s04-options .rd-v2-btn, .s04-actions .rd-v2-btn"],
      ["text", ".s04-options li > span"],
      ["term", ".s04-method dt"],
      ["value", ".s04-method dd"],
      ["band", ".s04-bands > b"],
      ["row", ".s04-decision-list > li > span > strong"],
      ["auth", ".s04-decision-list > li > b"],
      ["note", ".s04-fixture"],
      ["code", ".s04-code"],
    ];
    const out = [];
    for (const [kind, selector] of wanted) {
      for (const el of document.querySelectorAll(selector)) {
        const r = el.getBoundingClientRect();
        if (r.width < 3 || r.height < 3) continue;
        out.push({ kind, x: r.x + window.scrollX, y: r.y + window.scrollY,
                   w: r.width, h: r.height,
                   text: (el.getAttribute("data-testid") || el.innerText || "").trim().replace(/\s+/g, " ") });
      }
    }
    return out;
  });
  ...
  const minX = Math.min(...panels.map((n) => n.x)) - 12;
  const maxX = Math.max(...panels.map((n) => n.x + n.w)) + 12;
  const minY = Math.min(...panels.map((n) => n.y)) - 4;
  const maxY = Math.max(...panels.map((n) => n.y + n.h)) + 4;
  const COLS = 104, sx = COLS / (maxX - minX), sy = sx * 0.42;
  const ROWS = Math.ceil((maxY - minY) * sy) + 2;
  const grid = canvas(COLS + 2, ROWS);
  const X = (v) => Math.round((v - minX) * sx);
  const Y = (v) => Math.round((v - minY) * sy);

  for (const n of panels) box(grid, X(n.x), Y(n.y), Math.round(n.w * sx), Math.max(3, Math.round(n.h * sy)), n.text.replace("synthesis-", ""));
  for (const n of nodes.filter((e) => e.kind !== "panel")) {
    const mark = { btn: "[", band: "#", code: "`" }[n.kind] || "";
    const label = mark === "[" ? `[ ${n.text} ]` : mark === "#" ? "#".repeat(Math.max(1, Math.round(n.w * sx))) + " " + n.text : n.text;
    put(grid, X(n.x) + 1, Y(n.y + n.h / 2), label.slice(0, Math.max(0, COLS - X(n.x) - 2)));
  }
  console.log("\n\n=== LAYOUT AT 1440px, DRAWN FROM THE REAL BOXES ===\n");
  console.log(grid.map((row) => row.join("").replace(/\s+$/, "")).join("\n"));
