...
test("a ready provider becomes browsable only when a directory handler exists", () => {
  const locations = normalizeLibraryLocations([{ id: "google_drive", state: "ready", accountLabel: "Prof. Kong" }]);
...
test("provider requests are cursor-ready and bounded", () => {
  assert.deepEqual(providerDirectoryRequest({ providerId: "dropbox", parentId: "abc", cursor: "next" }), {
    provider: "dropbox",
    parent_id: "abc",
    cursor: "next",
